...
Once the runtime starts, you can access the project at [http://localhost:8080/jax-rs](http://localhost:8080/jax-rs).
...
```
./mvnw clean package
docker build -t jax-rs:v1 .
```

You can then run the Docker image by executing:

```
docker run -it --rm -p 8080:8080 jax-rs:v1
```
